...
Далее, добавим ещё один вывод, на этот раз для Twitch. Здесь также необходимо указать [`RTMP Сервер`](https://stream.twitch.tv/ingests/) и `RTMP` ключ трансляции, который можно найти в `Панели управления`, в разделе `Настройки` - `Канал`. Протестировать какой `RTMP` Сервер лучше всего подходит для вашего стрима, можно небольшой программой TwitchTest.
